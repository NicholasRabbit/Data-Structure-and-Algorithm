...
因为在根据BST的特性，有些情况下它有可能是一个单链表。这样的结构体现不出来二叉树的优势了，所以要想办法转化成AVL，使其各个节点平衡分步，这样查找修改的效率才会利用上二叉树的特性。
...
如何计算得到的$log_2n$ ? 结合个人代码MergeSortTest.java及下面思路图理解
1，设n=8, 则divide(...)函数需要平分3次，直到最后剩每组一个数，进行一对一比较；
2，merge(...)方法属于线性方法设时间为常数C，可以省去，则最后可得到$O(log_2n)$。
