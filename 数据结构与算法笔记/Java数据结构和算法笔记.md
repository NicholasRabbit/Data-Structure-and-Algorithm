...
- 完全二叉树(Complete Binary Tree): 最下层如果有叶子节点，那必须是坐叶子节点（下图 6号），倒数第二层如果有叶子节点那只能是右叶子节点（3号节点）（就是满二叉树最下层去掉一个右叶子节点）。完全二叉树包括满二叉树。
...
n：最后一个节点的下标，arr.length : 线索化后数组长度，也是二叉树中节点的个数。

(n-1)/2 或者arr.length/2 - 1 :    中间节点个数，同时也是最后一个中间节点的下标。arr.length = n + 1，公式换算后和前面的公式相同。

父节点和左右子树下标之间的关系（下标为n，从0开始，对应数组的下标）：
...
n个节点的二叉树中含有n + 1个空指针，推导公式：[2n - (n - 1)]。
